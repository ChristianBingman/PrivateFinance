...
def test_create_transaction(setup_example_accounts):
    ...
    xact_detail = TransactionDetail(description="A Sample Transaction", xact_date=datetime.now())
    xact_detail.save()
    from_account = Account.objects.get(name="Salary")
    to_account = Account.objects.get(name="Example Bank 1")

    entry1 = TransactionEntry(transaction_id=xact_detail, account=from_account, amount=decimal.Decimal('-10.00'))
    entry2 = TransactionEntry(transaction_id=xact_detail, account=to_account, amount=decimal.Decimal('10.00'))
...
def test_create_transaction_with_extra_precision(setup_example_accounts):
    ...
    entry1 = TransactionEntry.objects.create(transaction_id=xact_detail, account=from_account, amount=decimal.Decimal('-10.001'))
...
def test_invalid_transaction_amount(setup_example_accounts):
    ...
    entry1 = TransactionEntry(transaction_id=xact_detail, account=from_account, amount=decimal.Decimal('-10.00'))
    entry2 = TransactionEntry(transaction_id=xact_detail, account=to_account, amount=decimal.Decimal('1.00'))
...
def test_create_entries_multiple_accounts(setup_example_accounts):
    ...
    xact_detail = TransactionDetail(description="A Sample Transaction", xact_date=datetime.now())
    xact_detail_2 = TransactionDetail(description="A Different Transaction", xact_date=datetime.now())
    xact_detail.save()
    ...
    entry1 = TransactionEntry(transaction_id=xact_detail, account=from_account, amount=decimal.Decimal('-10.00'))
    entry2 = TransactionEntry(transaction_id=xact_detail_2, account=to_account, amount=decimal.Decimal('10.00'))
    with pytest.raises(ValueError):
        TransactionEntry.objects.create_balanced_transaction([entry1, entry2])
